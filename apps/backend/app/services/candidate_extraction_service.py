...
→ ``CandidateExtractionRepository``. The unit of work owns the session and the
single top-level transaction; neither the controller nor the service facade
touches ``AsyncSession`` / ``db.begin()`` / the session factory directly.

The publish enforces:
...
class CandidatePublishUnitOfWork:
    ...
    The unit of work is the only component that creates sessions and begins
    transactions; the controller and service facade never see ``AsyncSession``.
    """
