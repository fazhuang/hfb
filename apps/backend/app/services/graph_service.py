...
P0-4: cross_document_analysis — evidence-bound claims, no invented contradictions.
...
async def _validate_graph_evidence(
    ...
    """Validate evidence: chunk exists, belongs to document, quote is in chunk.
    ...
    # Verify quote is contiguous substring of chunk content
...
class GraphService:
    ...
    async def create_relation(
        ...
        err = await _validate_graph_evidence(
            self.session, evidence.document_id, evidence.chunk_id, evidence.exact_quote
        )
        if err is not None:
    ...
    async def _collect_all_edges(
        ...
        # --- 1. EntityRelation edges (only with valid evidence) ---
        er_stmt = (
            select(EntityRelation)
        ...
        for er in er_rows:
            # P0: skip orphan edges — must have valid structured evidence
        ...
        for entity_type, eids in sorted(nodes_by_type.items()):
            ...
            stmt = (
                select(model)
                .where(model.id.in_(sorted(eids)), model.is_deleted.is_(False))
                .order_by(model.id)
            )
            result = await self.session.execute(stmt)
    ...
    async def search_entities(
        ...
        for et in sorted(entity_types):
            ...
            if model is None:
                continue
            stmt = select(model).where(model.is_deleted.is_(False)).order_by(model.id)
            if query:
                if et == "person":
                    stmt = stmt.where(
                        or_(
    ...
    # ==================================================================
    # Sprint 3 P0-2: Concept Graph — corpus-endogenous
    # ==================================================================

    # Co-occurrence window: concepts must appear in the same chunk
    # to be considered co-occurring.
    ...
    async def build_concept_graph(self, concept_labels: list[str]) -> ConceptGraph:
        ...
        Only concepts that actually appear in chunks are included.
        Edges only exist when both concepts co-occur in at least one chunk.
        broader_than / narrower_than only created when explicit hierarchical
        markers appear in the evidence sentence.
        """
        ...
        for lbl in active_labels:
            ...
            # Evidence from first few chunks
            evidence = []
            for c in chunks[:5]:
        ...
        for i in range(len(active_labels)):
            for j in range(i + 1, len(active_labels)):
                ...
                # Find first shared chunk as evidence
                shared_chunk_id = sorted(shared)[0]
                evidence_chunk = next(c for c in all_chunks if c.id == shared_chunk_id)

                # Check for hierarchical markers
                hier_markers = ["包括", "包含", "属于", "是.*的一种", "分为"]
                has_hierarchy = any(
                    re.search(m, evidence_chunk.content) for m in hier_markers
                )
                sent = ""
                sentences = re.split(r"(?<=[。！？.!?])", evidence_chunk.content)
                for s in sentences:
    ...
    def concept_similarity(self, concept_a: str, concept_b: str) -> ConceptSimilarity:
        """Compute deterministic Jaccard co-occurrence similarity.
    ...
    # ==================================================================
    # Sprint 3 P0-4: Cross-Document Analysis
    # ==================================================================
    ...
    async def cross_document_analysis(self, topic: str) -> CrossDocumentAnalysis:
        ...
        Claims are extracted from chunks that contain the topic.
        Contradictions are only reported when two chunks make explicitly
        opposing statements (same subject, opposite polarity) and
        both are fully evidenced.
        """
        chunk_stmt = (
            select(DocumentChunk)
            .where(
        ...
        if not chunks:
            return CrossDocumentAnalysis(topic=topic)
        ...
        # Contradiction detection: opposing polarity markers
        contradictions: list[dict[str, CrossDocumentClaim]] = []
        neg_markers = ["不", "非", "未", "否", "无", "并非"]
        for i in range(len(supporting)):
            for j in range(i + 1, len(supporting)):
                a, b = supporting[i], supporting[j]
                if a.document_id == b.document_id:
                    continue
                a_neg = any(m in a.claim_text for m in neg_markers)
                b_neg = any(m in b.claim_text for m in neg_markers)
                if a_neg != b_neg:
