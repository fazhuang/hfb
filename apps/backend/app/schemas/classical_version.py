...
class ClassicalVersionCreate(BaseModel):
    ...
    edition_type: str | None = Field(
        default=None, max_length=100, description="版本类型"
    )
    ...
    source_url: str = Field(
        ..., min_length=1, max_length=2000, description="来源链接 — 必填"
    )
    image_url: str | None = Field(default=None, max_length=2000, description="书影链接")
    public_domain_status: str = Field(
        ..., max_length=50, description="公共领域状态 — 必填"
    )
    ...
    review_status: str = Field(
        default="pending_review", max_length=50, description="审核状态"
    )
