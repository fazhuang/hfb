...
def upgrade() -> None:
    op.create_table(
        ...
        sa.Column(
            "parent_id", sa.String(36), nullable=True, comment="自引用 — 回应另一条注疏"
        ),
        sa.Column(
            "relation_type",
            sa.String(20),
        ...
        sa.ForeignKeyConstraint(
            ["parent_id"], ["commentaries.id"], ondelete="SET NULL"
        ),
    )
    op.create_index(
        op.f("ix_commentaries_passage_id"), "commentaries", ["passage_id"], unique=False
    )
    op.create_index(
        op.f("ix_commentaries_parent_id"), "commentaries", ["parent_id"], unique=False
    )
