...
async def seed_graph(session: AsyncSession) -> dict[str, int]:
    ...
    async def _add(
        src_type: str, src_name: str,
        tgt_type: str, tgt_name: str,
    ...
    await _add("person", "皇甫谧", "book", "针灸甲乙经", "authored", "皇甫谧编撰《针灸甲乙经》")
    await _add("person", "张仲景", "book", "伤寒杂病论", "authored", "张仲景著《伤寒杂病论》")
    await _add("person", "李时珍", "book", "本草纲目", "authored", "李时珍著《本草纲目》")

    # ---- Person → Person (studied) ----
    # 李时珍 studied 张仲景's work
    if "李时珍" in persons and "张仲景" in persons:
        await _add("person", "李时珍", "person", "张仲景", "studied", "李时珍对张仲景《伤寒论》有深入研究")
    if "李时珍" in persons and "皇甫谧" in persons:
        await _add("person", "李时珍", "person", "皇甫谧", "studied", "李时珍引用皇甫谧《针灸甲乙经》的腧穴体系")

    # ---- Book → Book (cited_in) ----
    if "本草纲目" in books and "伤寒杂病论" in books:
        await _add("book", "本草纲目", "book", "伤寒杂病论", "cited_in", "《本草纲目》引用《伤寒杂病论》方剂")
    if "本草纲目" in books and "针灸甲乙经" in books:
        await _add("book", "本草纲目", "book", "针灸甲乙经", "cited_in", "《本草纲目》引用《针灸甲乙经》腧穴")
    if "针灸甲乙经" in books and "伤寒杂病论" in books:
        await _add("book", "针灸甲乙经", "book", "伤寒杂病论", "cited_in", "《针灸甲乙经》参考《伤寒杂病论》辨证体系")
