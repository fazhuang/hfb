...
async def main():
    async with async_session_factory() as session:
        ...
        r = await session.execute(text(
            "SELECT id FROM persons WHERE name='皇甫谧' AND is_deleted=false"))
        huangfu_mi_id = r.scalar_one()

        r = await session.execute(text(
            "SELECT id FROM books WHERE title='针灸甲乙经' AND is_deleted=false"))
        ...
        # Check existing
        r = await session.execute(text(
            "SELECT COUNT(*) FROM entity_relations WHERE is_deleted=false"))
        if r.scalar() > 0:
        ...
        ev1 = GraphEvidence(
            document_id=c1[1], chunk_id=c1[0], exact_quote=q1,
            citation=f'[{c1[1]}:{c1[0]}]',
            version_id=version_id, passage_id=c1[2],
            source_uri=src_uri, claim_text="皇甫谧编撰了《针灸甲乙经》")
        rel1 = await gs.create_relation(
            "person", huangfu_mi_id, "book", jiayi_book_id, "compiled", evidence=ev1)
        await gs.verify_relation(rel1.id,
            claim_text="皇甫谧编撰了《针灸甲乙经》",
            evidence_document_id=c1[1], evidence_version_id=version_id,
            evidence_passage_id=c1[2], evidence_chunk_id=c1[0],
            evidence_quote=q1, evidence_source_uri=src_uri, verified_by=admin_id)
        ...
        ev2 = GraphEvidence(
            document_id=c2[1], chunk_id=c2[0], exact_quote=q2,
            citation=f'[{c2[1]}:{c2[0]}]',
            version_id=version_id, passage_id=c2[2],
            source_uri=src_uri,
        ...
        await gs.verify_relation(rel2.id,
            claim_text="《针灸甲乙经》以《素问》《灵枢》《明堂孔穴针灸治要》三书为蓝本",
            evidence_document_id=c2[1], evidence_version_id=version_id,
            evidence_passage_id=c2[2], evidence_chunk_id=c2[0],
            evidence_quote=q2, evidence_source_uri=src_uri, verified_by=admin_id)
        print(f'Rel2: {rel2.id} verified')

        # --- Rel 3-5: 针灸甲乙经 --contains--> passages ---
        passages_evidence = [
            (c3, "皇甫谧以'使事类相从，删其浮辞，除其重复，论其精要'为编纂原则，使《针灸甲乙经》成为系统化的针灸学经典。",
             "《针灸甲乙经》的编纂原则为'使事类相从，删其浮辞，除其重复，论其精要'"),
            (c4, "该书确定了349个腧穴的位置、主治和针刺深度，为后世针灸腧穴标准化奠定了基础。",
             "《针灸甲乙经》确定了349个腧穴的位置、主治和针刺深度"),
            (c5, "《针灸甲乙经》强调经脉理论与脏腑辨证相结合，奠定了针灸治疗学的理论基础。",
             "《针灸甲乙经》强调经脉理论与脏腑辨证相结合"),
        ]
        for i, (c, quote, claim) in enumerate(passages_evidence, 3):
            ev = GraphEvidence(
                document_id=c[1], chunk_id=c[0], exact_quote=quote,
                citation=f'[{c[1]}:{c[0]}]',
                version_id=version_id, passage_id=c[2],
                source_uri=src_uri, claim_text=claim)
            rel = await gs.create_relation(
                "book", jiayi_book_id, "passage", c[2], "contains", evidence=ev)
            await gs.verify_relation(rel.id,
                claim_text=claim,
                evidence_document_id=c[1], evidence_version_id=version_id,
                evidence_passage_id=c[2], evidence_chunk_id=c[0],
                evidence_quote=quote, evidence_source_uri=src_uri,
