...
class Paper(BaseModel):
    ...
    title_english: Mapped[str | None] = mapped_column(String(1000), nullable=True, comment="英文标题")
    authors: Mapped[str | None] = mapped_column(Text, nullable=True, comment="作者 (JSON array string)")
    journal: Mapped[str | None] = mapped_column(String(500), nullable=True, comment="期刊/会议名")
    year: Mapped[int | None] = mapped_column(Integer, nullable=True, comment="发表年份")
    doi: Mapped[str | None] = mapped_column(String(500), nullable=True, unique=True, comment="DOI")
    ...
    full_text: Mapped[str | None] = mapped_column(Text, nullable=True, comment="全文文本")
