...
relation_evidences = Table(
    "relation_evidences",
    Base.metadata,
    Column("relation_id", String(36), ForeignKey("academic_relations.id", ondelete="CASCADE"), primary_key=True),
    Column("evidence_id", String(36), ForeignKey("evidences.id", ondelete="CASCADE"), primary_key=True)
)
...
class AcademicRelation(BaseModel):
    ...
    source_entity_id: Mapped[str] = mapped_column(
        ForeignKey("academic_entities.id", ondelete="CASCADE"), nullable=False, comment="源实体ID"
    )
    ...
    target_entity: Mapped[AcademicEntity] = relationship("AcademicEntity", foreign_keys=[target_entity_id])
...
class RelationConfidence(BaseModel):
    ...
    relation_id: Mapped[str] = mapped_column(
        ForeignKey("academic_relations.id", ondelete="CASCADE"), unique=True, nullable=False, comment="关联的学术命题"
    )
