...
class Sentence(BaseModel):
    ...
    order: Mapped[int] = mapped_column(
        Integer, nullable=False, comment="句子在段落内的序号"
    )

    # 关系
    passage: Mapped[Passage] = relationship("Passage", back_populates="sentences")
    tokens: Mapped[list[Token]] = relationship(
        "Token", back_populates="sentence", cascade="all, delete-orphan"
    )
...
class Token(BaseModel):
    ...
    __table_args__ = (Index("idx_token_lookup", "sentence_id", "position"),)
    ...
    char_text: Mapped[str] = mapped_column(
        String(50), nullable=False, comment="单个汉字或核心词"
    )
    position: Mapped[int] = mapped_column(
        Integer, nullable=False, comment="字/词在句中的绝对位置索引"
    )
    ...
    variants_as_base: Mapped[list[Variant]] = relationship(
        "Variant",
        foreign_keys="Variant.base_token_id",
        back_populates="base_token",
        lazy="selectin",
    )
    variants_as_compare: Mapped[list[Variant]] = relationship(
        "Variant",
        foreign_keys="Variant.compare_token_id",
        back_populates="compare_token",
        lazy="selectin",
    )
...
class Variant(BaseModel):
    ...
    variant_type: Mapped[VariantType] = mapped_column(
        Enum(VariantType), nullable=False, comment="异文类型"
    )
    description: Mapped[str | None] = mapped_column(
        Text, nullable=True, comment="校勘记/校勘说明"
    )

    # 关系
    base_token: Mapped[Token] = relationship(
        "Token", foreign_keys=[base_token_id], back_populates="variants_as_base"
    )
    compare_token: Mapped[Token | None] = relationship(
        "Token", foreign_keys=[compare_token_id], back_populates="variants_as_compare"
    )
