  ...
  <div class="lit-detail-page">
    ...
    <div v-else-if="doc" class="detail-content">
      ...
      <section class="panel compliance-panel">
        ...
        <div class="compliance-grid">
          ...
          <div class="field">
            ...
            <span class="badge" :class="`badge-review-${doc.review_status}`">{{
      ...
      <section v-if="doc.content_text" class="panel">
        ...
        <button class="expand-fulltext-btn" @click="contentExpanded = !contentExpanded">
      ...
      <section class="panel">
        ...
        <div class="meta-grid">
          <div class="field">
            <span class="field-label">拼音</span><span>{{ doc.title_pinyin || '—' }}</span>
          </div>
          <div class="field">
            <span class="field-label">英文</span><span>{{ doc.title_english || '—' }}</span>
          </div>
          <div class="field">
            <span class="field-label">年份</span><span>{{ doc.year || '—' }}</span>
          </div>
          <div class="field">
            <span class="field-label">语言</span><span>{{ doc.language }}</span>
          </div>
          <div class="field">
            <span class="field-label">页数</span><span>{{ doc.page_count || '—' }}</span>
          </div>
          <div class="field">
          ...
          <div class="field">
            ...
            ><span class="mono">{{ doc.content_checksum || '—' }}</span>
          </div>
        </div>
      </section>
      ...
      <section v-if="auth.canReviewDocuments" class="panel admin-panel">
        ...
        <div v-if="!doc.withdrawn_at" class="action-group">
          ...
          <div class="action-row">
            ...
            <button class="btn btn-primary" :disabled="reviewLoading" @click="submitReview">
              {{ reviewLoading ? '...' : t('literature.submitReview') }}
            </button>
          </div>
        ...
        <div class="action-group">
          ...
          <div class="action-row">
            <button
              v-if="!doc.rag_enabled"
              class="btn btn-secondary"
              :disabled="ragLoading"
              @click="toggleRag(true)"
            >
              启用智能检索
            </button>
            <button
        ...
        <div v-if="!doc.withdrawn_at" class="action-group action-group--danger">
          ...
          <div class="action-row">
            <input
              ...
              type="text"
              class="action-input"
              :placeholder="t('literature.withdrawReasonPlaceholder')"
            />
            <button class="btn btn-danger" :disabled="withdrawLoading" @click="submitWithdraw">
              {{ withdrawLoading ? '...' : t('literature.confirmWithdraw') }}
            </button>
          </div>
          <p v-if="withdrawMsg" class="action-msg" :class="{ 'msg-ok': withdrawOk }">
            {{ withdrawMsg }}
          </p>
        </div>
      </section>
...
async function submitReview() {
  ...
  reviewMsg.value = '';
  try {
    await api.patch(`/api/v1/documents/${doc.value.id}/review`, {
    ...
    reviewMsg.value =
      (e as { response?: { data?: { detail?: string } } })?.response?.data?.detail ?? '审核失败';
  } finally {
...
async function submitWithdraw() {
  ...
  try {
    ...
    withdrawMsg.value =
      (e as { response?: { data?: { detail?: string } } })?.response?.data?.detail ?? '撤回失败';
  } finally {
