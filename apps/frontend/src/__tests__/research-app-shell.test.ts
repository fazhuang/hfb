...
describe('ResearchPrimaryNav', () => {
  ...
  it('displays the four research module navigation items', async () => {
    ...
    expect(labelTexts).toContain('Research');
    expect(labelTexts).toContain('Library');
    expect(labelTexts).toContain('Knowledge');
    expect(labelTexts).toContain('Reports');
  });

  it('does NOT display Dashboard, Workspace, or Graph in the nav', async () => {
    const router = await createMockRouteAndNavigate('research');
    const wrapper = mount(ResearchPrimaryNav, {
      global: { plugins: [router] },
      props: { collapsed: false },
    ...
    expect(allText).not.toContain('Graph');
  });
  ...
  it('activates Research nav item when section is "research"', async () => {
    ...
    expect(activeLabels.some((l) => l.includes('Research'))).toBe(true);
  });
  ...
  it('activates Library nav item when section is "library"', async () => {
    ...
    const activeLinks = wrapper.findAll('.rpn-link--active');
    const activeLabels = activeLinks.map((el) => el.find('.rpn-link-label')?.text() || el.text());
    expect(activeLabels.some((l) => l.includes('Library'))).toBe(true);
  });
  ...
  it('activates Knowledge nav item when section is "knowledge"', async () => {
    ...
    const activeLinks = wrapper.findAll('.rpn-link--active');
    const activeLabels = activeLinks.map((el) => el.find('.rpn-link-label')?.text() || el.text());
    expect(activeLabels.some((l) => l.includes('Knowledge'))).toBe(true);
  });
  ...
  it('activates Reports nav item when section is "reports"', async () => {
    ...
    const activeLinks = wrapper.findAll('.rpn-link--active');
    const activeLabels = activeLinks.map((el) => el.find('.rpn-link-label')?.text() || el.text());
    expect(activeLabels.some((l) => l.includes('Reports'))).toBe(true);
  });

  it('has Administration as a visually separated section', async () => {
    const router = await createMockRouteAndNavigate('research');
    const wrapper = mount(ResearchPrimaryNav, {
      global: { plugins: [router] },
      props: { collapsed: false },
    ...
    // Administration link should exist in the nav
    expect(wrapper.text()).toContain('Administration');
    // Separator element should exist
...
// ================================================================
// Test 4: Router — all 5 research pages accessible via routes
// ================================================================
...
describe('Research App Shell Routing', () => {
  ...
  for (const { name, path } of pageCases) {
    it(`resolves "${path}" to ${name}`, async () => {
      ...
      // Verify the matched component is correct
      const matched = route.matched[route.matched.length - 1];
      expect(matched).toBeDefined();

      // Component is registered on the matched route
  ...
  it('sets section meta to "research" on all research pages', async () => {
    ...
    const route = router.currentRoute.value;
    // Meta from parent route propagates through matched
  ...
  it('activates Research nav for /research/123/workflow sub-route', async () => {
    ...
    const researchActive = activeLinks.some((el) => {
      ...
      return label.exists() && label.text() === 'Research';
    });
