...
function makeChunkId(idx: number) {
  return `chunk-${String(idx).padStart(8, '0')}-0000-0000-0000-000000000001`;
}
function makeOcrId(idx: number) {
  return `ocr-${String(idx).padStart(8, '0')}-0000-0000-0000-000000000001`;
}
function makePassageId(idx: number) {
  return `passage-${String(idx).padStart(8, '0')}-0000-0000-0000-000000000001`;
}
function makeCitId(idx: number) {
  return `cit-${String(idx).padStart(8, '0')}-0000-0000-0000-000000000001`;
}
function makeEvId(idx: number) {
  return `ev-${String(idx).padStart(8, '0')}-0000-0000-0000-000000000001`;
}

const C1 = makeChunkId(1),
  C2 = makeChunkId(2);
const OCR1 = makeOcrId(1),
  OCR2 = makeOcrId(2);
const P1 = makePassageId(1),
  P2 = makePassageId(2);
  ...
  citations: [
    {
      ...
      anchor_passage_ids: [P1],
    },
    {
      id: CIT_OCR,
      quote_text: 'OCR识别的扫描文本',
      note: 'OCR段落引用',
      target_type: 'Passage',
      target_id: P2,
      evidence_id: E_OCR,
      anchor_chunk_ids: [OCR1],
      anchor_passage_ids: [P2],
    },
  ],
  evidences: [
    {
    ...
    {
      id: E_OCR,
      description: 'OCR文本证据，来自扫描版针灸甲乙经',
      evidence_level: 3,
...
describe('ReaderPage', () => {
  ...
  it('16. shows translation-unavailable hint when passages exist but none translated', async () => {
    const noTrans = {
      ...mockReaderData,
      passages: mockReaderData.passages.map((p: Record<string, unknown>) => ({
        ...p,
        translation: null,
      })),
    };
