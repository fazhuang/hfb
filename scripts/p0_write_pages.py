...
def get_page_text(pdata):
    if isinstance(pdata, str): return pdata
...
def match_chunk_to_best_page(chunk_content, page_texts):
        ...
        for plen in [60, 40, 25, 15, 10]:
            ...
            for skip in [1, 2, 3]:
                ...
                if pos2 >= 0 and len(pref2) >= 6:
                    ...
                    break
            if best and best[0] == pg: break

        # LCS
        lcs, _, off = find_lcs(cnorm, pnorm)
        if len(lcs) >= 6:
            score = round(min(1.0, len(lcs) / max(1, len(cnorm))), 4)
            if best is None or score > best[3]: best = (pg, f"lcs_{len(lcs)}", off, score)
    ...
    detail = {
        ...
        "offset": offset,
        "best_score": score,
        "char_overlap_scores": {str(p): round(v, 3) for p, v in sorted(char_scores, key=lambda x: -x[1])[:5]} if char_scores else {},
    }
...
async def write_pages():
    async with async_session_factory() as s:
        ...
        """))
        chunks = [{"id": row[0], "doc_id": row[1], "idx": row[2], "content": row[3],
                    "old_page": row[4], "passage_id": row[5], "passage_order": row[6]}
        ...
        # Print results
        print(f"\n{'='*90}")
        print(f"{'Idx':>3s}  {'old_pg':>6s}  {'new_pg':>6s}  {'score':>7s}  {'provenance':22s}  {'ihash':14s}  {'content_snippet'}")
        print(f"{'='*90}")
        for r in results:
