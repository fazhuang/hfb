#!/usr/bin/env python3
"""Final build: merge all scrapling results + build PDF."""
...
def load_chapters():
    ...
    for fname in sorted(os.listdir(RAW_DIR)):
        if not fname.startswith("call_00_") or not fname.endswith(".txt"):
            continue
        if fname == "all_chapters.json": continue
    ...
    # Also load from all_chapters.json if it exists
...
def build():
    ...
    # Now also save all_chapters for reuse
    cleaned = {}
    ...
    for seq, (ch_id, label) in enumerate(CANONICAL):
        text = cleaned.get(ch_id, {}).get("content", "")
        if not text:
        ...
        toc_items.append(f'<li><a href="#{anchor}">{dl}</a></li>')
...
<style>
  ...
  *,*::before,*::after{{box-sizing:border-box;margin:0;padding:0;}}
  ...
  nav li{{margin:.2rem 0;font-size:.92rem;}}
...
{"".join(body_sections)}
    ...
    with open(OUT_HTML, "w", encoding="utf-8") as f:
        f.write(html)
    print(f"HTML: {OUT_HTML} ({len(html):,} bytes, {found} chapters)")
