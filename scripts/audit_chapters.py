#!/usr/bin/env python3
"""Build PDF from all available scrapling data, including inline results.

Strategy: all 91 chapters have been fetched. Persist missing data to disk
as individual JSON result files, then run normal build.
"""
...
for fname in sorted(os.listdir(RAW_DIR)):
    if fname.startswith('call_00_') and fname.endswith('.txt'):
        with open(os.path.join(RAW_DIR, fname)) as f:
            d = json.load(f)
            for item in d.get('result',[]):
                cid = item['url'].split('/')[-1]
                if cid: chapters[cid] = item

print(f'Disk chapters: {len(chapters)}')
...
# Write a file that build script can reference
with open(os.path.join(RAW_DIR, 'pending_chapters.txt'), 'w') as f:
    for m in missing:
        f.write(f'{m}\n')

print('Written pending_chapters.txt')
