#!/usr/bin/env python3
"""Build complete 针灸甲乙经 PDF.

NOTE: All inline scrapling results from the 4 bulk_stealthy_fetch calls
(53 chapters worth) are passed via the ALL_INLINE_JSON env var or
a separate all_chapters.json file that must be written first.

Run: python3 scripts/build_complete_v2.py
"""
...
CANONICAL = [
    ...
    ("1m1jdip68fw32", "右刺禁", "卷五"),
    ("1jurkz32bnos3", "鍼灸甲乙經卷六", None),
    ("1m1jdir0ytee2", "八正八虛八風大論第一", "卷六"),
    ...
    ("1m1jdivamhnou", "大寒內薄骨髓陽逆發頭痛第一", "卷九"),
    ("1jurkz4zowmxx", "邪在肺五藏六府受病發欬逆上氣第三", "卷九"),
    ...
    ("1lzguzkr198gq", "邪氣聚於下脘發內癰第八", "卷十一"),
    ("1m1jdiyhtokrq", "鍼灸甲乙經卷十二", None),
    ("1m1jdiyhtoxeu", "客氣客於厭發喑不能言第二", "卷十二"),
...
def load_all_chapters() -> OrderedDict:
    chapters = OrderedDict()
    with open(ALL_JSON, "r", encoding="utf-8") as f:
        raw = json.load(f)
    for ch_id, data in raw.items():
        cleaned = clean_markdown(data.get("content", ""))
        chapters[ch_id] = {"content": cleaned, "url": data.get("url", "")}
    return chapters
...
def build_html(chapters: OrderedDict) -> str:
    toc_items = []
    body_sections = []
    last_vol = None

    for seq, (ch_id, label, vol) in enumerate(CANONICAL):
        ...
        if vol and vol != last_vol:
            last_vol = vol
            # vol_divider shown inside section via CSS
...
def main():
    ...
    print(f"Loaded: {found}/{expected} chapters")
    if missing:
