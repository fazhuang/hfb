...
def ocr_pages_if_needed(start: int = 1, end: int = 40, dpi: int = OCR_DPI):
    ...
    for pg in range(start, end + 1):
        ...
        if (
            ...
            and existing[k].get("ocr_text", "").strip()
        ):
            continue
        ...
        existing[k] = {
            ...
            "ocr_dpi": dpi,
            "ocr_text": full_text,
            "ocr_norm_text": normalize(full_text),
            "ocr_avg_confidence": avg_conf,
            "n_lines": len(texts),
            "chinese_chars": chinese_chars(full_text),
...
FACT_SOURCE_MAP = [
    ...
    },
    {
        ...
        "pdf_page": 4,
        "source_quote": (
            "黄帝内經十八卷今有鍼經九卷素问九卷二九十八卷即内經也...明堂孔穴鍼灸治要...至為十二卷"
        ),
    ...
    },
    {
        ...
        "pdf_page": 7,
        "source_quote": (
            "鍼灸甲乙經卷之一...凡刺之法必先本於神血脉營氣精神此五藏之所藏"
        ),
    ...
    },
    {
        ...
        "pdf_page": 8,
        "source_quote": (
            "肝藏血血舍魂...心藏脉脉舍神...脾藏營營舍意...肺藏氣氣舍魄...是故五藏主藏精"
        ),
...
async def run_pipeline(dry_run: bool = False):
    ...
    assert pdf_sha == PDF_SHA256, (
        f"PDF checksum mismatch: {pdf_sha[:16]} != {PDF_SHA256[:16]}"
    )
    ...
    async with async_session_factory() as session:
        ...
        if doc[2]:  # no blob yet
            ...
            await session.flush()
            print(
                f"    Stored PDF blob ({len(pdf_bytes)} bytes), pdf_sha256={pdf_sha[:16]}..."
            )
        else:
            await session.execute(
                text(
    ...
    async with async_session_factory() as session:
        ...
        await session.flush()
        print(
            f"    Created {chunks_created} OCR-backed chunks (pages {min(ocr_pages.keys())}-{max(ocr_pages.keys())})"
        )
    ...
    async with async_session_factory() as session:
        ...
        for i, fm in enumerate(FACT_SOURCE_MAP):
            ...
            source_ref_id_val = (
                rel_data[7] if rel_data and rel_data[7] else source_ref_id
            )
        ...
        for i, f in enumerate(facts_audit, 1):
            ...
            for key in [
                ...
                print(f"  {key:24s}: {val}")
            print(
                f"  {'quote_offset_or_bbox':24s}: {json.dumps(f['quote_offset_or_bbox'], ensure_ascii=False)[:120]}..."
            )
        ...
        print(f"\n{'=' * 80}")
        print(
            f"RESULT: {sum(1 for f in facts_audit if f['match_result'])}/{len(facts_audit)} match_result=true"
        )
        print(f"{'=' * 80}")
