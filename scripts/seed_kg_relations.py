...
async def main():
    async with async_session_factory() as session:
        ...
        r = await session.execute(
            text("SELECT id FROM versions WHERE is_deleted=false LIMIT 1")
        )
        version_id = r.scalar()
        ...
        r = await session.execute(
            text(
                "SELECT id FROM passages WHERE content_text LIKE '%使事类相从%' AND is_deleted=false"
            )
        )
        ...
        r = await session.execute(
            text(
                "SELECT id FROM passages WHERE content_text LIKE '%349个腧穴%' AND is_deleted=false"
            )
        )
        ...
        r = await session.execute(
            text(
                "SELECT id FROM passages WHERE content_text LIKE '%经脉理论与脏腑辨证%' AND is_deleted=false"
            )
        )
