...
async def main():
    ...
    async with async_session_factory() as session:
        ...
        print(
            f"refusal={before.refusal} citations={len(before.citations)} kg_paths={len(before.kg_paths)}"
        )
        for i, c in enumerate(before.citations[:5]):
            print(
                f"  cit[{i}] doc={c.document_id[:20]} chunk={c.chunk_id[:20]} quote={c.exact_quote[:60]}"
            )
        ...
        await ing.withdraw_document(
            pdf_doc_id, reason="P0 withdraw test", actor_id=admin_id
        )
        ...
        print(
            f"refusal={after.refusal} citations={len(after.citations)} kg_paths={len(after.kg_paths)}"
        )
        wd = [c for c in after.citations if c.document_id == pdf_doc_id]
        print(f"withdrawn_doc_citations={len(wd)}")
        print(
            f"VERDICT={'PASS' if (after.refusal or len(after.citations) == 0 or len(wd) == 0) else 'FAIL'}"
        )
