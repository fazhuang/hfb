...
async def main():
    ...
    async with async_session_factory() as session:
        ...
        for i, c in enumerate(b.citations[:5]):
            print(
                f"  CIT[{i}]: doc={c.document_id[:20] if c.document_id else '?'} "
        ...
        r = await session.execute(
            text("SELECT withdrawn_at FROM documents WHERE id = :did"),
            {"did": pdf_doc_id},
        )
        wa = r.scalar_one()
        ...
        verdict = (
            "PASS" if (a.refusal or len(a.citations) == 0 or len(wd) == 0) else "FAIL"
        )
        ...
        if not a.refusal and a.citations:
            for i, c in enumerate(a.citations[:5]):
                print(
                    f"  AFTER_CIT[{i}]: doc={c.document_id[:20] if c.document_id else '?'} "
                    f"q={c.exact_quote[:40] if c.exact_quote else '?'}"
                )
