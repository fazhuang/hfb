#!/usr/bin/env python3
"""Build complete 针灸甲乙经 PDF from online scrape data.

Reads scrapling results from all_chapters.json on disk.
Dynamically fetches any missing chapters.
"""
...
def load_chapters():
    ...
    for ch_id, data in chapters.items():
        cleaned[ch_id] = {"content": clean_markdown(data["content"]), "url": data.get("url","")}
    return cleaned
...
def build():
    ...
    missing = [cid for cid, _ in CANONICAL if cid not in chapters]
    if missing:
        print(f"WARNING: {len(missing)} chapters still missing from disk:")
        print(f"  Missing IDs: {missing[:10]}..." if len(missing) > 10 else f"  {missing}")
        print("  These chapters were fetched inline but not persisted to disk.")
        print("  Re-run the scrapling calls first, or the PDF will be incomplete.")
