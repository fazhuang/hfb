...
async def main():
    ...
    async with async_session_factory() as s:
        ...
        for i, row in enumerate(rows):
            ...
            if nq not in nc:
                ...
                print(
                    f"  Fact {i + 1}: WARNING - quote not exact in chunk (overlap={overlap:.1%})"
                )
                if overlap < 0.7:
    ...
    async with async_session_factory() as s:
        ...
        for i, c in enumerate(resp.citations[:10]):
            ...
            print(
                f"    quote: {str(c.exact_quote)[:100] if c.exact_quote else 'None'}..."
            )
            print(f"    evidence_id: {c.evidence_id}")
            print(
                f"    source_uri: {str(c.source_uri)[:80] if c.source_uri else 'None'}..."
            )
