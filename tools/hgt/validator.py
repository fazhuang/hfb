def validate_scan(result):
    ...
    if result.duplicate_document_ids:
        errors.append(f"Found duplicate document_id groups: {len(result.duplicate_document_ids)}")
    if result.known_duplicate_candidates:
        errors.append(f"Found known legacy duplicate files: {len(result.known_duplicate_candidates)}")
