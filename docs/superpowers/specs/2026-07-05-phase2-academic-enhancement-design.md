...
class Commentary(BaseModel):
    ...
    commentary_type: Mapped[str]  # interlinear_gloss(夹注) | end_of_passage(段后注) | sub_commentary(疏) | commentary_work(独立注疏专著) | critique(评议)
    ...
    relation_type: Mapped[Optional[str]]  # supplements(补充) | refutes(驳斥) | expands(阐发) | annotates(注释) | interprets(阐释前人注)
