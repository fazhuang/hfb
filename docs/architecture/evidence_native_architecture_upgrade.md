...
> **版本**：v7.1 (2026年8月第十六次架构审计后严格末行编校与全字段重放规范)  
> **状态**：编校规则严格限定于末行 / 补全字符 Span 审计 Log / 离线全字段物理重放校验通过 / 准备进入 Phase A0 后端编码  
...
### 1.1 转型背景与核心判断
经过 2026 年 8 月对所有外部参考项目（Google LangExtract, PaperWeave, Vercel Guidelines, Graphify, Dokploy 等）的十六次深入复核与第一性原理工程总结，HFB 确定了关键的发展方向决策：
...
4. **严格末行限定规则与全字段离线物理重放**：
   - 面对上游 Wikisource Raw HTML 中同处一个 `<p>` 标签内的裸文本分类，编校规则（`rule-wikisource-tail-category-v1.0`）**严格限定仅在全篇最后一个非空行（Last Non-Empty Line）精准匹配时才剥离**！
   - 中间正文行若包含同形字符，一律 100% 完好保留！
   - 审计日志补全 `raw_dom_start`、`raw_dom_end` 字符偏移 Span，以及 `input_sha256`、`output_sha256`；
   - 离线 `--offline` 校验提升为**全字段物理重放比对**（重放 Raw DOM -> Rule Execution -> Canonical Body -> 审计 Log 逐字段验证），`python -O` 免疫。
...
### 5.1 Editorial Mode 严格末行编校基线快照
数据由 `scripts/clean_wikisource_body.py` (`hfb-strict-tail-editorial-pipeline-v9.0`) 固定 `oldid=794138` 幂等生成并落盘于 `tests/fixtures/gold_benchmark_v03.json`：
* **源出处 API**：Wikisource 卷 03 (revid: `794138`)
* **解析清洗器**：`scripts/clean_wikisource_body.py` (`hfb-strict-tail-editorial-pipeline-v9.0`)
...
* **Canonical Editorial 字符数**：`16,690` (经严格末行编校规则剥离，真 SHA-256: `f85f17381aefbfa9577a74fdde490eef8d3a523f748a1e4a210c789de86d6cbe`)
* **规则审计 Log**：完整记有 `raw_dom_start`: `16691`、`raw_dom_end`: `16705`、`stripped_text`: `<子部,醫家類,鍼灸甲乙經>`、`input_sha256` 与 `output_sha256`
...
v7.1 规范实现了严格末行限定规则、带字符 Span 审计 Log 以及离线全字段物理重放校验，基线工具资产彻底达到顶级可审计生产标准。
