...
> **例外记录（HFB-DEV-0505-EXC-01，Phase A0）**：「证据原生」候选发布管线专用
> 表 `candidate_audit_logs` 与 `candidate_extractions`。
> 1. `candidate_audit_logs`（模型 `CandidateAuditLog`）继承 `Base` 而非 `BaseModel`
