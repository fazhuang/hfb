...
*   **是否支持后续知识图谱:**
    *   系统包含了 `AcademicEntity`、`AcademicRelation`、`RelationConfidence` 等实体与关联表示模型（参见 [__init__.py](file:///Users/likeming/Sites/hfb/apps/backend/app/models/__init__.py)）。采集到的文献信息（如人物、机构、研究主题）可直接映射进知识图谱。
*   **是否支持后续校勘研究:**
