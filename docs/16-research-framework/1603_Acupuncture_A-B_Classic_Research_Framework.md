---
title: Research Framework for The Systematic Classic of Acupuncture and Moxibustion
document_id: HFB-RF-1603
version: 1.0.0
status: Approved
owner: Academic Committee
reviewer: Chief Scientific Advisor
effective_date: 2026-06-24
scope: Research Framework for The Systematic Classic of Acupuncture and Moxibustion
priority: P0
related_documents:
  - HFB-RF-1601 Digital Humanities Research Framework
  - HFB-RF-1602 Huangfu Mi Studies Framework
  - HFB-DOM-0802 Book Knowledge Model
  - HFB-DOM-0803 Version Knowledge Model
  - HFB-DOM-0809 Master Knowledge Graph Model
---

# Research Framework for The Systematic Classic of Acupuncture and Moxibustion

## 《针灸甲乙经》研究框架

> 本框架定义《皇甫谧数字人文与中医经典智能研究平台》关于《针灸甲乙经》的总体研究体系。
>
> 平台不仅实现古籍数字化，更以数字人文方法重构《针灸甲乙经》的**文本、知识、传播、思想、影响**五个维度，形成可持续演进的智能研究体系。

---

# 第一章 学术定位

《针灸甲乙经》是中国现存最早的针灸学系统专著之一，也是中医针灸理论发展的重要里程碑。

平台定位：

> **以《针灸甲乙经》为核心对象，建设国际领先的数字化、知识化、智能化研究平台。**

---

# 第二章 总体研究目标

平台围绕《针灸甲乙经》建立八大研究方向：

```text
文本研究

↓

版本研究

↓

知识研究

↓

医学思想研究

↓

传播研究

↓

学术研究

↓

数字人文研究

↓

人工智能研究
```

形成全生命周期研究体系。

---

# 第三章 文本研究体系

重点内容：

- 原始文本整理
- 结构分析
- 章节体系
- 条文体系
- 文本来源
- 文本演化

成果：

建立全文 Passage 数据库。

---

# 第四章 版本学研究体系

平台建立：

Version Registry。

包括：

- 已知版本目录
- 刊刻系统
- 收藏机构
- 数字资源
- 版本谱系
- 校勘关系

形成：

Version Genealogy。

---

# 第五章 校勘学研究体系

建立：

Collation Database。

记录：

- 异文
- 缺文
- 衍文
- 倒文
- 校改意见
- 校勘依据

所有校勘均关联具体 Passage。

---

# 第六章 医学知识体系

平台建立知识对象：

- 经络
- 腧穴
- 病证
- 针法
- 灸法
- 治法
- 理论

形成医学知识图谱。

---

# 第七章 医学思想研究

重点分析：

- 整体观
- 辨证思想
- 经络理论
- 针灸理论
- 临床原则

支持思想演化分析。

---

# 第八章 文献来源研究

建立：

Source Knowledge Graph。

追溯：

- 《黄帝内经》
- 《明堂经》
- 《难经》
- 《素问》
- 《灵枢》

分析文本继承关系。

---

# 第九章 传播研究体系

建立：

Transmission Graph。

包括：

- 中国传播
- 东亚传播
- 世界传播
- 现代数字传播

支持传播路径重建。

---

# 第十章 地域研究体系

重点分析：

- 甘肃
- 陕西
- 河南
- 日本
- 韩国
- 欧洲

自动生成：

地域研究专题报告。

---

# 第十一章 学术史研究

建立：

Research History Timeline。

统计：

- 研究阶段
- 热点变化
- 代表人物
- 代表机构
- 国际合作

支持学术演化分析。

---

# 第十二章 AI 研究体系

AI 提供：

- 条文检索
- 多版本比对
- 自动校勘辅助
- 医学术语识别
- 学术综述生成
- 引文分析

所有回答必须引用原始文献。

---

# 第十三章 数字孪生建设

构建：

Digital Twin of 《针灸甲乙经》。

包括：

- 全文数字化
- 全版本数字化
- 全知识对象数字化
- 全关系数字化

形成完整数字镜像。

---

# 第十四章 知识图谱体系

统一图谱对象：

```text
Book

↓

Version

↓

Passage

↓

Acupoint

↓

Disease

↓

Treatment

↓

Person

↓

Paper
```

形成多层知识网络。

---

# 第十五章 GraphRAG 体系

GraphRAG 默认流程：

```text
问题

↓

实体识别

↓

Passage 检索

↓

版本映射

↓

知识图谱推理

↓

证据排序

↓

引用生成

↓

AI 回答
```

---

# 第十六章 数字人文分析

平台支持：

- 文本网络分析
- 引文网络分析
- 学术传播分析
- 地域聚类分析
- 时间演化分析

形成数字人文研究成果。

---

# 第十七章 国际研究体系

建设：

International Research Hub。

包括：

- 多语言版本
- 国际论文库
- 国际学者网络
- 海外馆藏信息

推动国际合作研究。

---

# 第十八章 平台核心创新

形成八项创新能力：

1. 全版本知识库
2. Passage 级知识管理
3. 自动版本比勘
4. 医学知识图谱
5. 学术传播分析
6. AI 学术问答
7. AI 自动综述
8. 《针灸甲乙经》数字孪生

---

# 第十九章 长期目标

平台最终形成：

> **《针灸甲乙经》全球数字研究中心（Global Digital Research Center for The Systematic Classic of Acupuncture and Moxibustion）**

为中医经典研究、针灸教育、国际传播及 AI 医学研究提供基础设施。

---

# 第二十章 修订规则

修改本框架必须同步更新：

- Version Knowledge Model
- Passage Knowledge Model
- Knowledge Graph Model
- Academic Citation Model
- GraphRAG Specification

未经学术委员会批准不得修改。

---

# 修订记录

| Version | Date       | Description                                        |
| ------- | ---------- | -------------------------------------------------- |
| 1.0.0   | 2026-06-24 | 首版发布，作为《针灸甲乙经》数字人文研究总体框架。 |
