...
from app.repositories.entities import (
    BookRepository, VersionRepository, PassageRepository, PaperRepository, ImageRepository,
)
...
class TestVersionRepository:
    ...
    async def test_create(self, db_session: AsyncSession):
        ...
        v = await repo.create(book_id="b", version_name="北宋刻本", era="宋")
...
class TestPassageRepository:
    ...
    async def test_search(self, db_session: AsyncSession):
        ...
        await repo.create(chapter_id="c1", content_text="凡刺之法，必候日月星辰")
        await repo.create(chapter_id="c2", content_text="黄帝问曰：针道可得闻乎")
