...
def _seed_passage_with_lineage(
    db, passage_id: str, content: str, version_name: str = "宋本"
):
    ...
    book = Book(
        id=f"book-{passage_id}", title="针灸甲乙经", source_url="http://example.com"
    )
    db.add(book)
    version = Version(
        id=f"ver-{passage_id}",
...
def test_internal_trace_record_all_fields_required():
    ...
    for k in d:
        ...
        if k != "retrieval_score":
            assert d[k] != "", (
                f"{k} should not be empty (except retrieval_score which can be 0)"
            )
...
def test_traceability_block_query_id_min_length():
    ...
    with pytest.raises(ValidationError):
        V4TraceabilityBlock(
            query_id="", trace_ids=[], citation_count=0, source_documents=[]
        )
...
def test_visualization_schema_rejects_empty():
    ...
    with pytest.raises(ValidationError):
        VisualizationEdge(
            source="a", target="b", type="co_occurrence", weight=0.5, evidence_ids=[]
        )
    with pytest.raises(ValidationError):
...
async def test_query_with_passage_returns_lineage(db_session_persistent):
    ...
    pid = _seed_passage_with_lineage(
        db_session_persistent, "passage-qa", "经络是运行气血的通道。"
    )
    _seed_chunks_with_passage(
        db_session_persistent,
        "v4-doc-qa",
        "针灸甲乙经",
        "晋",
        passage_id=pid,
    ...
    async with AsyncClient(transport=transport, base_url="http://test") as client:
        r = await client.post(
            "/api/v4/research/session", json={"title": "p", "query": "经络"}
        )
...
async def test_query_unmapped_passage_fail_closed(db_session_persistent):
    ...
    db_session_persistent.add(
        DocumentChunk(
            id="v4-chk-nop-0",
            document_id=doc.id,
            chunk_index=0,
            content="针灸和经络是中医核心概念。",
            token_count=12,
        )
    )
    await db_session_persistent.flush()
    ...
    async with AsyncClient(transport=transport, base_url="http://test") as client:
        r = await client.post(
            "/api/v4/research/session", json={"title": "unmapped", "query": "针灸"}
        )
        body = r.json()
...
async def test_resolver_strict_missing_passage_fails(db_session_persistent):
            ...
            result_summary=json.dumps(
                {
                    "traces": [
                        {
                            "trace_id": tid,
                            "chunk_id": "v4-chk-str-0",
                            "document_id": "v4-doc-str",
                            "passage_id": "",
                            "retrieval_score": 0.5,
    ...
    assert "passage_id" in str(exc.value).lower() or "TRACE_LINEAGE_INCOMPLETE" in str(
        exc.value
    )
...
async def test_workflow_e2e_with_passage(db_session_persistent):
    ...
    pid = _seed_passage_with_lineage(
        db_session_persistent, "passage-wf", "经络是运行气血的通道。"
    )
    _seed_chunks_with_passage(
        db_session_persistent,
        "v4-doc-wf",
...
async def test_visualization_session_bound(db_session_persistent):
    ...
    pid = _seed_passage_with_lineage(
        db_session_persistent, "passage-vs", "针灸和经络是中医核心概念。"
    )
    _seed_chunks_with_passage(
        db_session_persistent,
        "v4-doc-vs",
    ...
    async with AsyncClient(transport=transport, base_url="http://test") as client:
        ...
        r_sess = await client.post(
            "/api/v4/research/session", json={"title": "viz session"}
        )
...
async def test_history_traceability_carries_real_trace_ids(db_session_persistent):
    ...
    pid = _seed_passage_with_lineage(
        db_session_persistent, "passage-hist", "环周不休。"
    )
    _seed_chunks_with_passage(
        db_session_persistent,
        "v4-doc-hist",
    ...
    async with AsyncClient(transport=transport, base_url="http://test") as client:
        r1 = await client.post(
            "/api/v4/research/session", json={"title": "h", "query": "经络"}
        )
        sid = r1.json()["data"]["session_id"]
...
async def test_timeline_uses_structured_time_not_regex(db_session_persistent):
    ...
    pid = _seed_passage_with_lineage(
        db_session_persistent, "passage-tlv", "无年代文本。"
    )
    _seed_chunks_with_passage(
        db_session_persistent,
        "v4-doc-tlv",
    ...
    async with AsyncClient(transport=transport, base_url="http://test") as client:
        r_sess = await client.post(
            "/api/v4/research/session", json={"title": "timeline"}
        )
        sid = r_sess.json()["data"]["session_id"]
        ...
        for edge in graph["edges"]:
            assert False, (
                f"Timeline with no structured time evidence should not have edges: {edge}"
            )
...
async def test_citation_graph_excludes_hierarchy_cooccurrence(db_session_persistent):
    ...
    pid = _seed_passage_with_lineage(
        db_session_persistent, "passage-cit2", "针灸和经络密切相关。"
    )
    _seed_chunks_with_passage(
        db_session_persistent,
        "v4-doc-cit2",
...
async def test_citation_graph_has_edges_with_evidence(db_session_persistent):
    ...
    pid = _seed_passage_with_lineage(
        db_session_persistent, "passage-cit3", "针灸和经络密切相关。"
    )
    _seed_chunks_with_passage(
        db_session_persistent,
        "v4-doc-cit3a",
    ...
    async with AsyncClient(transport=transport, base_url="http://test") as client:
        r_sess = await client.post(
            "/api/v4/research/session", json={"title": "cit-real"}
        )
        sid = r_sess.json()["data"]["session_id"]
        r = await client.post(
            "/api/v4/visualization/graph",
        ...
        assert len(graph["edges"]) > 0, (
            "Citation graph must have edges when evidence exists"
        )
        for edge in graph["edges"]:
            assert edge["type"] == "citation"
            assert len(edge["evidence_ids"]) > 0, (
                "Each citation edge must have evidence_ids"
            )
...
async def test_citation_graph_empty_without_evidence(db_session_persistent):
    ...
    async with AsyncClient(transport=transport, base_url="http://test") as client:
        r_sess = await client.post(
            "/api/v4/research/session", json={"title": "cit-empty"}
        )
        sid = r_sess.json()["data"]["session_id"]
        r = await client.post(
            "/api/v4/visualization/graph",
...
async def test_education_beginner_all_filters_consistent(db_session_persistent):
    ...
    pid = _seed_passage_with_lineage(
        db_session_persistent, "passage-edu2", "经络系统包括十二经脉。"
    )
    _seed_chunks_with_passage(
        db_session_persistent,
        "v4-doc-edu2",
    ...
    for fpath in v4_files:
        ...
        for imp in imports:
            assert not imp.startswith("app.models"), (
                f"{fpath} imports ORM models: {imp}"
            )
...
def test_snapshot_preserves_real_score():
    ...
    rr = RetrievalResult(
        ...
        score=0.83,
        metadata={"retrieval_method": "ili_keyword_search"},
    )
    snapshot = {"chk-1": rr}
...
async def test_visualization_query_history_has_full_traces(db_session_persistent):
    ...
    pid = _seed_passage_with_lineage(
        db_session_persistent, "passage-viz-full", "经络内容。"
    )
    _seed_chunks_with_passage(
        db_session_persistent,
        "v4-doc-vizf",
    ...
    async with AsyncClient(transport=transport, base_url="http://test") as client:
        r_sess = await client.post(
            "/api/v4/research/session", json={"title": "viz trace"}
        )
        sid = r_sess.json()["data"]["session_id"]
...
async def test_graph_mode_saves_query_history(db_session_persistent):
    ...
    pid = _seed_passage_with_lineage(
        db_session_persistent, "passage-gmode", "针灸经络。"
    )
    _seed_chunks_with_passage(
        db_session_persistent,
        "v4-doc-gmode",
...
async def test_graph_mode_evidence_lineage_nonempty_on_sqlite(db_session_persistent):
    ...
    pid = _seed_passage_with_lineage(
        db_session_persistent, "passage-t1b", "经络针灸甲乙经。"
    )
    ...
    async with AsyncClient(transport=transport, base_url="http://test") as client:
        ...
        body = r2.json()
        assert body["success"] is True, (
            f"graph mode must succeed: {body.get('message')}"
        )
...
@pytest.mark.asyncio
async def test_visualization_api_citation_count_matches_query_history(
    db_session_persistent,
):
    ...
    pid = _seed_passage_with_lineage(
        db_session_persistent, "passage-count", "经络内容。"
    )
    _seed_chunks_with_passage(
        db_session_persistent,
        "v4-doc-count",
...
@pytest.mark.asyncio
async def test_replay_twice_produces_byte_identical_canonical_output(
    db_session_persistent,
):
    ...
    pid = _seed_passage_with_lineage(
        db_session_persistent, "passage-replay2", "经络内容。"
    )
    _seed_chunks_with_passage(
        db_session_persistent,
        "v4-doc-replay2",
    ...
    frozen_traces = [
        InternalTraceRecord(
            trace_id=tid,
            document_id="v4-doc-replay2",
            chunk_id="v4-chk-replay2-000",
            passage_id=pid,
            provenance_kind="retrieval",
            retrieval_score=0.85,
            retrieval_method="ili_keyword",
...
async def test_replay_modified_snapshot_produces_mismatch(db_session_persistent):
    ...
    frozen_traces = [
        InternalTraceRecord(
            trace_id=tid,
            document_id="v4-doc-mod2",
            chunk_id="v4-chk-mod2-000",
            passage_id=pid,
            provenance_kind="retrieval",
            retrieval_score=0.85,
            retrieval_method="ili_keyword",
    ...
    assert h1 != h2, (
        f"Modified snapshot must produce different hash. original={h1}, modified={h2}"
    )
...
def test_canonicalize_trace_score_method_kind_changes_result():
    ...
    assert canonicalize_trace(base) != canonicalize_trace(
        {**base, "retrieval_score": 0.01}
    )
    assert canonicalize_trace(base) != canonicalize_trace(
        {**base, "retrieval_method": "fabricated-method"}
    )
    assert canonicalize_trace(base) != canonicalize_trace(
        {**base, "provenance_kind": "graph"}
    )
...
def test_canonical_traces_enter_all_hash_domains():
    ...
    h1_input = canonical_sha256(
        _build_input_payload(
            "T", "full", "1.0", snapshot, ["a"], ["d1"], canonical_traces=canonical
        )
    )
    h1_output = canonical_sha256(
        _build_canonical_payload(
            "T",
            "full",
            "1.0",
    ...
    h2_output = canonical_sha256(
        _build_canonical_payload(
            ...
            ["a"],
            ["d1"],
            canonical_traces=canonical_fab,
        )
    )
    ...
    # Modify score/method
    traces_mod = [
        {**traces[0], "retrieval_score": 0.01, "retrieval_method": "fabricated"}
    ]
    canonical_mod = canonicalize_traces(traces_mod)

    h3_input = canonical_sha256(
        _build_input_payload(
            "T", "full", "1.0", snapshot, ["a"], ["d1"], canonical_traces=canonical_mod
        )
    )
    h3_output = canonical_sha256(
        _build_canonical_payload(
            "T",
            "full",
            "1.0",
            ...
            ["a"],
            ["d1"],
            canonical_traces=canonical_mod,
        )
    )
...
async def test_replay_api_normal_workflow_replay_twice(db_session_persistent):
    ...
    pid = _seed_passage_with_lineage(
        db_session_persistent, "passage-replay-api", "经络内容。"
    )
    _seed_chunks_with_passage(
        db_session_persistent,
        "v4-doc-replay-api",
...
async def test_replay_api_tampered_passage_id_fails(db_session_persistent):
    ...
    pid = _seed_passage_with_lineage(
        db_session_persistent, "passage-tamp-pid", "经络。"
    )
    _seed_chunks_with_passage(
        db_session_persistent,
        "v4-doc-tamp-pid",
    ...
    async with AsyncClient(transport=transport, base_url="http://test") as client:
        ...
        runs = (await client.get(f"/api/v4/research/session/{sid}/runs")).json()[
            "data"
        ]["runs"]
        run_id = runs[0]["run_id"]
        ...
        assert "CORRUPT_MANIFEST" in body.get("data", {}).get(
            "error", ""
        ) or "REPRODUCIBILITY" in body.get("message", "")
...
async def test_replay_api_tampered_retrieval_score_fails(db_session_persistent):
    ...
    pid = _seed_passage_with_lineage(
        db_session_persistent, "passage-tamp-score", "经络。"
    )
    _seed_chunks_with_passage(
        db_session_persistent,
        "v4-doc-tamp-score",
...
async def test_replay_api_manifest_sha256_tamper_fails(db_session_persistent):
    ...
    async with AsyncClient(transport=transport, base_url="http://test") as client:
        ...
        body = r4.json()
        assert body["success"] is False
        assert "manifest_sha256" in json.dumps(
            body["data"]
        ).lower() or "CORRUPT" in str(body["data"])
...
async def test_replay_api_manifest_sha256_required(
    db_session_persistent, mode, replacement, expected
):
...
async def test_visualization_auto_session_returns_session_id(db_session_persistent):
    ...
    pid = _seed_passage_with_lineage(
        db_session_persistent, "passage-viz-auto", "经络。"
    )
    _seed_chunks_with_passage(
        db_session_persistent,
        "v4-doc-viz-auto",
