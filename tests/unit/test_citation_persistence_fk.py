...
async def _seed_source_ref(db, sr_id: str, url: str = "http://example.com/book/1",
                           title: str = "测试文献", page_location: str = "") -> str:
...
async def _seed_passage(db, passage_id: str, version_id: str, content: str = "经络是运行气血的通道。") -> str:
    ...
    await db.execute(text(
        ...
        "ON CONFLICT DO NOTHING"
    ), {"id": passage_id, "chapter_id": chapter_id, "version_id": version_id, "content": content})
    await db.flush()
    return passage_id


async def _seed_document_chunk(db, chunk_id: str, doc_id: str, passage_id: str = "",
                                content: str = "经脉流行不止，环周不休。") -> str:
    ...
    await db.execute(text(
        ...
        "tokens": len(content), "passage_id": passage_id or None,
    })
    await db.flush()
    return chunk_id
...
async def test_persist_citation_three_table_fk_join(db_session_persistent):
    ...
    chk_id = await _seed_document_chunk(db_session_persistent, "chk-join-1", doc_id, pid)
    ...
    join_result = await db_session_persistent.execute(text("""
...
async def test_backfill_missing_source_refs(db_session_persistent):
    ...
    await db_session_persistent.execute(text(
    ...
    result = await db_session_persistent.execute(text(
    ...
    """))
    assert join_result.fetchone() is not None, "3-table JOIN must succeed after backfill"
