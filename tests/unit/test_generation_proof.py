...
def _make_retrieval_result(document_id="d1", chunk_id="c1", content="经络者。", score=0.9):
...
class TestGenerationProof:
    def test_default_is_not_complete(self) -> None:
        resp = GroundedGenerationResponse(query="test", answer="", results=[], citations=[])
...
class TestErrorCodes:
    def test_all_known_codes(self) -> None:
        expected = [
            "ACADEMIC_CLAIM_BINDING_FAILED", "CLAIM_COUNT_MISMATCH",
            "CITATION_COUNT_MISMATCH", "CHUNK_NOT_IN_SNAPSHOT",
            "DOCUMENT_ID_MISMATCH", "CITATION_MALFORMED",
            "QUOTE_NOT_CONTIGUOUS_SUBSTRING", "ANSWER_NOT_DETERMINISTIC",
...
class TestRenderCanonicalAnswer:
    ...
    def test_single_claim(self) -> None:
        cc = CanonicalClaim(quote="经络者", document_id="d1", chunk_id="c1",
                            citation="[d1:c1]", chunk_rank=0, start_pos=0, quote_norm="经络者")
        result = _render_canonical_answer((cc,))
        assert "经络者" in result
        assert "[d1:c1]" in result

    def test_punctuation_appended(self) -> None:
        cc = CanonicalClaim(quote="无标点文字", document_id="d2", chunk_id="c2",
                            citation="[d2:c2]", chunk_rank=1, start_pos=5, quote_norm="无标点文字")
        result = _render_canonical_answer((cc,))
...
class TestBuildAndValidateProofRefusal:
    def test_evidence_gate_refusal(self) -> None:
        ...
        outcome = GenerationOutcome(response=resp, canonical_claims=(), snapshot={}, chunk_rank={})
    ...
    def test_claim_count_mismatch(self) -> None:
        cc = CanonicalClaim(quote="经络者。", document_id="d1", chunk_id="c1",
                            citation="[d1:c1]", chunk_rank=0, start_pos=0, quote_norm="经络者")
        resp = GroundedGenerationResponse(
            query="test", answer="no citation markers here",
            results=[{"document_id": "d1", "chunk_id": "c1", "chunk_index": 0,
                      "content": "经络者。", "citation": "[d1:c1]", "score": 0.9,
                      "source_url": "", "copyright_status": "public_domain"}],
            citations=[{"document_id": "d1", "chunk_id": "c1", "text": "[d1:c1]"}],
        )
        outcome = GenerationOutcome(response=resp, canonical_claims=(cc,), snapshot={}, chunk_rank={})
        proof = build_and_validate_proof(outcome)
...
class TestBuildAndValidateProofErrors:
    ...
    def test_document_id_mismatch(self) -> None:
        ...
        outcome = GenerationOutcome(response=resp, canonical_claims=(cc,), snapshot={"c1": rr}, chunk_rank={})
        proof = build_and_validate_proof(outcome)
    ...
    def test_citation_malformed(self) -> None:
        cc = CanonicalClaim(quote="经络者。", document_id="d1", chunk_id="c1",
                            ...
                            chunk_rank=0, start_pos=0, quote_norm="经络者")
        rr = _make_retrieval_result(document_id="d1", chunk_id="c1")
        resp = GroundedGenerationResponse(
            query="test", answer="经络者。[d2:c1]",
            results=[{"document_id": "d1", "chunk_id": "c1", "chunk_index": 0,
                      "content": "经络者。", "citation": "[d1:c1]", "score": 0.9,
                      "source_url": "", "copyright_status": "public_domain"}],
            citations=[{"document_id": "d1", "chunk_id": "c1", "text": "[d1:c1]"}],
    ...
    def test_quote_not_contiguous_substring(self) -> None:
        cc = CanonicalClaim(quote="不存在的文字", document_id="d1", chunk_id="c1",
                            citation="[d1:c1]", chunk_rank=0, start_pos=0, quote_norm="不存在的文字")
        rr = _make_retrieval_result(document_id="d1", chunk_id="c1", content="经络者。")
        resp = GroundedGenerationResponse(
            query="test", answer="不存在的文字[d1:c1]",
            results=[{"document_id": "d1", "chunk_id": "c1", "chunk_index": 0,
                      "content": "经络者。", "citation": "[d1:c1]", "score": 0.9,
                      "source_url": "", "copyright_status": "public_domain"}],
            citations=[{"document_id": "d1", "chunk_id": "c1", "text": "[d1:c1]"}],
    ...
    def test_answer_not_deterministic(self) -> None:
        ...
        rr = _make_retrieval_result(document_id="d1", chunk_id="c1")
        resp = GroundedGenerationResponse(
            query="test", answer="different text [d1:c1]",
            results=[{"document_id": "d1", "chunk_id": "c1", "chunk_index": 0,
                      "content": "经络者。", "citation": "[d1:c1]", "score": 0.9,
                      "source_url": "", "copyright_status": "public_domain"}],
            citations=[{"document_id": "d1", "chunk_id": "c1", "text": "[d1:c1]"}],
    ...
    def test_complete_proof(self) -> None:
        ...
        outcome = GenerationOutcome(response=resp, canonical_claims=(cc,), snapshot={"c1": rr}, chunk_rank={"c1": 0})
        proof = build_and_validate_proof(outcome)
