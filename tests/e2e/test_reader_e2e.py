...
def _get_reader_data(backend_port, access_token, doc_id):
    ...
    r = httpx.get(
        f"{base}/api/v1/documents/{doc_id}/reader", headers=headers, timeout=10
    )
    assert r.status_code == 200, (
        f"Reader endpoint must return 200, got {r.status_code}: {r.text[:300]}"
    )
...
class TestReaderE2E:
    ...
    def test_reader_refresh_restores_text(
        ...
        assert page.locator(f"text={doc_title}").count() > 0, (
            "Title must be visible before refresh"
        )
        page.reload()
        page.wait_for_timeout(5000)
        ...
        body = page.locator("body").first.text_content() or ""
        assert doc_title in body, (
            f"Title must persist after refresh. Body: {body[:300]}"
        )
    ...
    def test_reader_original_text_sourced_from_backend(
        ...
        assert len(original_chunks) >= 2, (
            f"Fixture must create 2+ original chunks, got {len(original_chunks)}"
        )
    ...
    def test_reader_translation_missing_display(
        ...
        if not has_translation and passages:
            assert "暂无现代汉语翻译" in body, (
                f"Must show translation-unavailable hint. Body: {body[:300]}"
            )
    ...
    def test_reader_stable_chunk_ordering(
        ...
        assert indices == sorted(indices), (
            f"Original chunk indices must be sorted: {indices}"
        )
        ...
        assert len(chunk_elements) > 0, (
            "Must have chunk DOM elements with id='chunk-...'"
        )
    ...
    def test_citation_precise_anchor_click_and_highlight(
        ...
        anchored_cit = next(
            (
                c
                for c in citations
                if c.get("anchor_chunk_ids") and len(c["anchor_chunk_ids"]) > 0
            ),
            None,
        )
        ...
        anchor_btn = page.locator(
            f"#citation-{anchored_cit['id']} .reader-anchor-btn"
        ).first
        ...
        assert chunk_el.count() > 0, (
            f"Target chunk element #chunk-{real_chunk_id} must exist in DOM"
        )
    ...
    def test_citation_hash_refresh_restores_anchor(
        ...
        page.wait_for_timeout(
            500
        )  # brief settle for scrollIntoView + highlight to apply

        # Verify chunk is highlighted on initial load
        chunk_el = page.locator(f"#chunk-{real_chunk_id}").first
        assert chunk_el.count() > 0, (
            f"Chunk #chunk-{real_chunk_id} must exist after hash navigation"
        )
        chunk_classes = chunk_el.get_attribute("class") or ""
        assert "reader-highlight" in chunk_classes, (
        ...
        assert chunk_el2.count() > 0, (
            f"Chunk #chunk-{real_chunk_id} must exist after refresh"
        )
    ...
    def test_citation_no_anchor_degrades_cleanly(
        ...
        if unanchored is None:
            unanchored = next(
                (c for c in citations if not c.get("anchor_chunk_ids")), None
            )
        if unanchored is None:
        ...
        assert cit_section.count() > 0, (
            f"Citation container #citation-{unanchored_cit_id} must exist"
        )
        ...
        assert anchor_btn.count() == 0, (
            "Unanchored citation must NOT have anchor button"
        )
    ...
    def test_evidence_precise_anchor_click_and_highlight(
        ...
        anchored_ev = next(
            (
                ev
                for ev in evidences
                if ev.get("anchor_chunk_ids") and len(ev["anchor_chunk_ids"]) > 0
            ),
            None,
        )
        ...
        anchor_btn = page.locator(
            f"#evidence-{anchored_ev['id']} .reader-anchor-btn"
        ).first
    ...
    def test_evidence_no_anchor_degrades_cleanly(
        ...
        if fixture_ev_id:
            unanchored = next(
                (ev for ev in evidences if ev["id"] == fixture_ev_id), None
            )
        if unanchored is None:
            unanchored = next(
                (ev for ev in evidences if not ev.get("anchor_chunk_ids")), None
            )
        if unanchored is None:
            ...
            assert anchored_ev is not None, (
                "Need at least one evidence for negative test"
            )
            _login_via_ui(page, frontend_url, a["username"], "LibA_Pass123!")
            _goto_reader(page, frontend_url, doc_id)
        ...
        assert ev_section.count() > 0, (
            f"Evidence container #evidence-{unanchored_ev_id} must exist"
        )
        ...
        assert anchor_btn.count() == 0, (
            "Unanchored evidence must NOT have anchor button"
        )
    ...
    def test_reader_http_422_shows_error_state(
        ...
        assert error_component.count() > 0, (
            "ErrorState component must be visible for 422"
        )
    ...
    def test_reader_http_500_shows_error_state(
        ...
        error_component = page.locator('[role="alert"]').first
        assert error_component.count() > 0, (
            "ErrorState component must be visible for 500"
        )
...
class TestReaderCrossUserIsolation:
    ...
    def test_user_a_cannot_access_user_b_reader_api(
        ...
        r = httpx.get(
            f"{base}/api/v1/documents/{b_doc_id}/reader", headers=headers, timeout=10
        )
    ...
    def test_tampered_doc_id_cannot_leak_other_user_data(
        ...
        # A can read own doc
        r = httpx.get(
            f"{base}/api/v1/documents/{a_doc_id}/reader", headers=headers, timeout=10
        )
        assert r.status_code == 200
...
class TestReaderRapidSwitching:
    ...
    def test_rapid_switch_backend_stale_response_ignored(
        ...
        assert r2.status_code == 404, (
            f"Non-existent doc must return 404, got {r2.status_code}"
        )
